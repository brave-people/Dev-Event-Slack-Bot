...
class Crawler:
    ...
    def get_html(self, target_url=None):
        ...
        """
        try:
            url = os.environ.get("event_check_url") if isinstance(
                target_url, type(None)) else target_url
            res = get(url, timeout=3)
            return BeautifulSoup(res.content, 'lxml')
        except Exception as exc:
            return exception_handler(exc, self.repository)
    ...
    def get_standard_data(self) -> list[str]:
        ...
        standard_data = self.repository.get_standard_data()
        if standard_data:
            return standard_data.get("issue_url_list")
        else:
            return self.repository.insert_standard_data(self.data_parsing()).get("issue_url_list")
    ...
    def set_standard_data(self, issue_url_list: list[str]) -> list[str]:
        ...
        #         writer.writerow([url])
        return self.repository.update_standard_data(issue_url_list).get("issue_url_list")
    ...
    def data_parsing(self) -> List[str]:
        ...
        try:
            ...
            issue_lists = main_div.find("div", {
                                        "class": "js-navigation-container js-active-navigation-container"}).find_all('div')
    ...
    def data_detail_parsing(self, target_url):
        ...
        try:
            ...
            txt_image = main_table.select_one(
                "tbody > tr:nth-child(1) > td > div > a").find('img').get('src')
            txt_tag = main_table.select_one(
                "tbody > tr:nth-child(1) > td > p:nth-child(3)")
    ...
    def data_compare(self) -> list[dict]:
        ...
        result_diff_list = list(
            set(new_issue_url_list) - set(standard_data_list))
